...
unsigned char keyboard(void){
        ...
        if (PINB.2 == 0)return 4;
        if (PINB.3 == 0)return 7;
        PORTB = 0b10110000; 
        if (PINB.0 == 0)return 0;
        if (PINB.1 == 0)return 2;
        if (PINB.2 == 0)return 5;
        if (PINB.3 == 0)return 8;                      
        PORTB = 0b11010000;
        if (PINB.1 == 0)return 3;
        if (PINB.2 == 0)return 6;
        if (PINB.3 == 0)return 9;                      
        else key =10;    
    PORTB = 0xf0;
    return key;
}
